...
model_test_data <- test_data %>%
  ...
  select( htr_Avg, atr_Avg, OffensiveDifference, DefensiveDifference,Home.OffensiveRating, Away.OffensiveRating,Home.DefensiveRating,Away.DefensiveRating, HomeGroundAdvantage)
# Make predictions
...
hasd <- test_data %>%
  mutate(correct_pick = case_when(Margin < 0 & PredictedMargin < 0 ~ 1,
                                  ...
                                  Margin == 0 & PredictedMargin == 0 ~ 1,
                                  TRUE ~ 0)) %>%
  mutate(         pr = map_margin_to_outcome(PredictedMargin), 
                  expr = map_elo_to_score(pr), 
                  mae_expr = abs(Margin - expr)) %>% 
  select(
    "Game", "Date", "Round", "Home.Team", "Away.Team", "Margin","PredictedMargin", pr, expr, mae_expr,correct_pick,
...
hasd %>%
  ...
  summarise(correct_pick = sum(correct_pick),
            ...
            mae_expr = sum(mae_expr)/allgames, 
            prc = correct_pick/allgames)
