...
exp_score_lm_hometest <- result_orig_withstats  %>% 
  ...
  lm(Home.Points ~ hshotsAtGoal + hInside.50s + hMarks.Inside.50 + hTackles +hTacklesi50 +
     hcontestedPossessions  + hclearances+ hhitouts + hturnovers, data = .)

exp_score_lm_awaytest <- result_orig_withstats %>% 
  filter(Date < '2023-01-01') %>% 
  mutate(away.scoring.shots = Away.Goals + Away.Behinds) %>% 
  lm(Away.Points ~  ashotsAtGoal + aInside.50s + aMarks.Inside.50 + aTackles + aTacklesi50 +
       acontestedPossessions  + aclearances+ ahitouts + aturnovers, data = .)
...
away_state23 <- f23 %>% 
  ...
  filter( away.state != "SA" | Away.team == "Adelaide Crows" ) %>%
...
fix_23clean <- f23 %>% 
  ...
  select("Date" = utcStartTime, "Season" = year ,  "Round" = round.roundNumber, 
         "Home.team"= home.team.name ,"hscore"= home.score.totalScore,    
         "Away.team" = away.team.name ,"ascore"= away.score.totalScore, 
