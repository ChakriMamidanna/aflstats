...
cleaned_data <- cleaned_data %>% 
  calculate_rolling_avg(., "Away.Team", c("Away.Tackles", "Away.ContestedPossessions", "Away.InterceptMarks")) %>%
...
# cleaned_data %>%
#   ggplot(aes(Margin, HomeGroundAdvantage)) + geom_point() + geom_smooth()
...
cleaned_data <- cleaned_data %>%
  ...
  select("Game", "Date", "Round", "Home.Team", "Home.Goals", "Home.Behinds", 
         ...
         "Venue",htr_Avg, atr_Avg, "hInside.50s", "hMarks.Inside.50", "hTackles", "hTacklesi50", 
